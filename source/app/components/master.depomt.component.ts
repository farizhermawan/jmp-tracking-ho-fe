...
export default class MasterDepoMTComponent extends DefaultPage {
  private view;
  private dataSorted;
  ...
  static Factory() {
    return {
      controller: MasterDepoMTComponent,
      templateUrl: 'views/components/master.depomt.html'
    };
  }
  ...
  toggleRecord(item) {
    let _this = this;
    _this.loading = true;
    _this.dataService.put('/v1/depo-mt/' + item.id, {'flag_active': !item.flag_active}).then(function () {
      _this.loadData();
    }, function () {
      _this.errorMsg("Error", "Gagal menyimpan data");
    });
  ...
  removeRecord(item) {
    ...
    this.confirmRemove(function () {
      _this.dataService.delete('/v1/depo-mt/' + item.id).then(function () {
        _this.reset();
        _this.loadData();
        _this.changeView('read');
      });
  ...
  private validateForm() {
    this.resetError();
    if (this.param.name == null || this.param.name == "") this.addError('name', 'Nama depo mt tidak boleh kosong');
    return !this.isError();
  }
}

MasterDepoMTComponent.$inject = ['dataService', '$filter', 'SweetAlert'];
